...
class NcryptEncryptor {
  ...
  Future<List<String>> encryptTestString() async {
    ...
    String encryptedTestString = encrypter.encrypt('secret test string', iv: iv).base64;
    return [encryptedTestString, iv.base64];
  }

  Future<bool> testPassword(Map<String, dynamic> testStringIV, String testedPassword) async {
    ...
    String testString;
    try {
      testString = testEncrypter.decrypt(Encrypted.fromBase64(testStringIV["test_string_encrypted"]), iv: iv);
    } catch(e) {
      print(e);
      return false;
    }
  ...
  Future<Account> decryptAccount(EncryptedAccount encryptedAccount) async {
    ...
    if (encryptedAccount.accountname.base64 != '') {
      decryptedAccountname = encrypter.decrypt(encryptedAccount.accountname, iv: accountNameIV);
    }

    IV usernameIV = IV.fromBase64(encryptedAccount.usernameIV);
    String decryptedUsername = '';
    if (encryptedAccount.username.base64 != '') {
      decryptedUsername = encrypter.decrypt(encryptedAccount.username, iv: usernameIV);
    }
    
    IV passwordIV = IV.fromBase64(encryptedAccount.passwordIV);
    String decryptedPassword = '';
    if (encryptedAccount.password.base64 != '') {
      decryptedPassword = encrypter.decrypt(encryptedAccount.password, iv: passwordIV);
    }
  ...
  Future<EncryptedAccount> encryptAccount(Account account) async {
    ...
    if (account.accountname != '') {
      encryptedAccountnameBase64 = encrypter.encrypt(account.accountname, iv: accountnameIV).base64;
    } 

    IV usernameIV = await generateIV();
    String encryptedUsernameBase64 = '';
    if (account.username != '') {
      encryptedUsernameBase64 = encrypter.encrypt(account.username, iv: usernameIV).base64;
    }

    IV passwordIV = await generateIV();
    String encryptedPasswordBase64 = '';
    if (account.password != '') {
      encryptedPasswordBase64 = encrypter.encrypt(account.password, iv: passwordIV).base64;
    }
  ...
  Future<List<Account>> decryptAccountList(List<EncryptedAccount> encryptedAccountList) async {
  ...
  Future<Note> decryptNote(EncryptedNote encryptedNote) async {
    ...
    if (encryptedNote.content.base64 != '') {
      decryptedContent = encrypter.decrypt(encryptedNote.content, iv: contentIV);
    }
  ...
  Future<EncryptedNote> encryptNote(Note note) async {
    ...
    if (note.content != '') {
      encryptedContentBase64 = encrypter.encrypt(note.content, iv: contentIV).base64;
    }
  ...
  Future<List<Note>> decryptNoteList(List<EncryptedNote> encryptedNoteList) async {
